...
async function showAccessUrl(port: int | 10058) {
  let prompt = "服务启动成功，请从下列URL访问本服务:\n";
  // 全部网卡信息
  const all = getNetworkAddrs();
  //console.table(all);

  // 获取真实IPv4列表，排除127.0.0.1
  const ips = getExternalIPv4(all);
  //console.log("本机IPv4：", ips);
  prompt += "\t===> http://localhost:" + port + "\n";
  ips.forEach((s, index) => {
    //prompt += "\t\thttp://" + s + ":" + port + "\n;
    prompt += "\t===> http://" + s + ":" + port + "\n";
  });
...
export default async function startServe(randomPort: Boolean = false) {
  clogger.info("Start Application...");
  ...
  clogger.info("oss静态资源文件目录:", ossDir);
  app.use(
    "/oss",
    (req, res, next) => {
  ...
  clogger.info("skills 静态资源文件目录:", skillsDir);
  // 只允许图片文件访问
  app.use(
    "/skills",
    (req, res, next) => {
  ...
  const port = randomPort ? 0 : process.env.APP_PORT;
  return await new Promise((resolve) => {
    server.listen(port, async () => {
      const address = server.address();
      const realPort = typeof address === "string" ? address : address?.port;

      //clogger.info(`服务启动成功: http://localhost:${realPort}`);
      await showAccessUrl(realPort);
